...
COLS = [
    ...
    ("Master tokens", lambda d: d.get("master_tok"), "#,##0"),
    ("Sub-agent tokens", lambda d: d.get("sub_tok"), "#,##0"),
    ("Master input", lambda d: d.get("master_in"), "#,##0"),
    ("Master output", lambda d: d.get("master_out"), "#,##0"),
    ("Sub-agent input", lambda d: d.get("sub_in"), "#,##0"),
    ("Sub-agent output", lambda d: d.get("sub_out"), "#,##0"),
...
LEGEND = [
    ...
    ("Input / Output tokens", "Gemini prompt (incl. cached) / output (candidates + thinking) tokens. BILLING-ACCURATE. Multi-agent agents: COMPLETE totals from Monitoring token_count (captures AgentTool sub-agent tokens the stream misses; undercount factors 1.00–1.41x)."),
    ("Master / Sub-agent tokens", "Input+output split into coordinator vs sub-agent/tool tokens, via the architecture-driven % from the two-model (3.5-flash/3.1-flash-lite) validation. Single-agent agents = 100% master; blank = split not measured."),
    ("Master/Sub input & output", "The master/sub totals broken into input vs output, via the measured per-role in:out ratio (master 88:12, sub 61:39, from the two-model token_count by model+type). master_in+sub_in = Input tokens; master_out+sub_out = Output tokens."),
