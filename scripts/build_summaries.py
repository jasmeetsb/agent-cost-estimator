...
docs/agent_summaries/<package>.md each, then docs/COMBINED_SKU_USAGE_REPORT.md.
"""
...
def combined(ds):
    ...
    (REPO / "docs" / "COMBINED_SKU_USAGE_REPORT.md").write_text("\n".join(L))
...
def master(ds):
    ...
    L += ["",
          ...
          "session storage GiB-hr; **Mem-gen tok** is priced at the input rate (single-rate proxy) and "
          "excludes the monthly per-memory storage charge.",
          "- **Proxy / lower bound:** **Web grounding** is billed per grounded prompt, but we count the "
          "web-research AgentTool invocation (internal multi-search would bill more).",
    ...
    L += ["",
          ...
          "_See also: [COMBINED_SKU_USAGE_REPORT.md](../COMBINED_SKU_USAGE_REPORT.md) (docs "
...
def main():
    ...
    combined(ds)
    master(ds)
    print("Wrote per-agent summaries + COMBINED_SKU_USAGE_REPORT.md + docs/agent_summaries/MASTER_SUMMARY.md")
